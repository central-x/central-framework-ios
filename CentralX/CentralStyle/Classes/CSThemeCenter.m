...
#import "CSThemeCenter.h"
#import <CentralX/CentralRuntime.h>

static NSString * CSCurrentThemeNameKey = @"CSCurrentThemeNameKey";

@interface CSThemeCenter()
@property (nonatomic, strong) NSMutableDictionary *registedThemes;
@property (nonatomic, strong) NSHashTable *observers;
@end

@implementation CSThemeCenter{
    CSTheme *_currentTheme;
...
+ (instancetype)center{
    static CSThemeCenter *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[CSThemeCenter alloc] _init];
    });
    return instance;
}

- (void)registerTheme:(CSTheme *)theme forName:(NSString *)themeName{
...
- (void)registerObserver:(id<CSThemeObserver>)observer{
    NSParameterAssert(observer != nil);
    [self.observers addObject:observer];
}

- (void)applyToObserver:(id<CSThemeObserver>)observer{
...
- (void)applyThemeWithName:(NSString *)themeName{
    ...
    CSTheme *theme = [self.registedThemes objectForKey:themeName];
    
    _currentTheme = theme;
    _currentThemeName = themeName;
    //保存主题信息
    [[NSUserDefaults standardUserDefaults] setObject:themeName forKey:CSCurrentThemeNameKey];
    
    dispatch_async(dispatch_get_main_queue(), ^{
        //应用主题
        for (id<CSThemeObserver> observer in self.observers) {
            [observer loadTheme:self->_currentTheme];
        }
    });
}

- (CSTheme *)currentTheme{
    if (!_currentTheme) {
...
- (NSString *)currentThemeName{
    return _currentThemeName ?: ({
        _currentThemeName = [[NSUserDefaults standardUserDefaults] objectForKey:CSCurrentThemeNameKey];
        if (_currentThemeName.length < 1) {
            _currentThemeName = [self.registedThemes allKeys][0];
            [[NSUserDefaults standardUserDefaults] setObject:_currentThemeName forKey:CSCurrentThemeNameKey];
...
- (void)autoRegisterClass:(Class)aClass beforeExecuting:(SEL)registeSEL applyBeforeExecuting:(SEL)applySEL{
    ...
    [CRAspect interceptSelector:registeSEL
        ...
        if ([invocation.target conformsToProtocol:@protocol(CSThemeObserver)]) {
            if (self.showLog) {
                NSLog(@"🌈🌈CSThemeCenter: Auto register instance: <%@ %p>\n", [invocation.target class], invocation.target);
            }
            [[CSThemeCenter center] registerObserver:invocation.target];
        }
        [invocation invoke];
    })];
    ...
    [CRAspect interceptSelector:applySEL
                ...
                withInterceptor:CRInterceptorMake(^(NSInvocation *invocation) {
        if ([invocation.target conformsToProtocol:@protocol(CSThemeObserver)]) {
            [invocation.target loadTheme:[CSThemeCenter center].currentTheme];
        }
        [invocation invoke];
    })];
}
